...
export function TweaksPanel({ title = 'Tweaks', noDeckControls = false, children }: TweaksPanelProps): JSX.Element | null {
  ...
  const [railEnabled, setRailEnabled] = useState<boolean>(
    () => {
      ...
      const el = document.querySelector('deck-stage') as (HTMLElement & { _railEnabled?: boolean }) | null;
      return !!el?._railEnabled;
    },
  );
  useEffect(() => {
...
export function TweakRow({ label, value, children, inline = false }: TweakRowProps): JSX.Element {
  return (
...
export function TweakSlider({ label, value, min = 0, max = 100, step = 1, unit = '', onChange }: TweakSliderProps): JSX.Element {
  return (
...
export function TweakRadio({ label, value, options, onChange }: TweakRadioProps): JSX.Element {
  const trackRef = useRef<HTMLDivElement>(null);
  const [dragging, setDragging] = useState<boolean>(false);
  const valueRef = useRef<string>(value);
  valueRef.current = value;

  const labelLen = (o: RadioOption): number => String(typeof o === 'object' ? o.label : o).length;
...
export function TweakSelect({ label, value, options, onChange }: TweakSelectProps): JSX.Element {
  return (
    <TweakRow label={label}>
      <select
        className="twk-field"
...
export function TweakText({ label, value, placeholder, onChange }: TweakTextProps): JSX.Element {
  return (
    <TweakRow label={label}>
      <input
        className="twk-field"
...
export function TweakNumber({ label, value, min, max, step = 1, unit = '', onChange }: TweakNumberProps): JSX.Element {
...
const __TwkCheck = ({ light }: { light: boolean }): JSX.Element => (
...
export function TweakColor({ label, value, options, onChange }: TweakColorProps): JSX.Element {
  ...
  const key = (o: string | string[]): string => String(JSON.stringify(o)).toLowerCase();
  const cur = key(value);
  return (
    <TweakRow label={label}>
...
export function TweakButton({ label, onClick, secondary = false }: TweakButtonProps): JSX.Element {
  return (
